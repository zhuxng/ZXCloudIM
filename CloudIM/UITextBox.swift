...
//    }
    ...
    private func animationFirstResponder(isFirstResponder:Bool) -> Bool {
        UIView.animate(withDuration: TimeInterval(animateDuration)) {
            ...
            self.placeholderLabel?.textColor = self.getTextColorWithHighlightColor(color: color)
        }
...
//        } catch { return nil }
//        let length = hex.distance(from: hex.startIndex, to: hex.endIndex)
...
//        }
//        
////        let start = hex.startIndex.advancedBy(result.rangeAt(1).length + result.rangeAt(1).location)
////        let start = hex[hex.index(hex.endIndex, offsetBy: result.rangeAt(1).length + result.rangeAt(1).location)]
//
//        let end = hex[hex.index(hex.endIndex, offsetBy: result.range.length + result.range.location)]
